...
Kirra exports and imports charge configurations as a **ZIP file** containing CSV files. All charge designs use **deck arrays** — there are no flat-field shortcuts. Every config is a template of typed deck entries plus primer entries, applied per-hole by the unified template engine.
