...
export class GeometryFactory {
    ...
    // Step 11) Create KAD polygon (closed line)
    ...
    static createKADCircle(worldX, worldY, worldZ, radius, lineWidth, color) {
        ...
        // Create just the outline (no fill)
        const points = [];
        const vertices = geometry.attributes.position;
        for (let i = 0; i < vertices.count; i++) {
            points.push(new THREE.Vector3(vertices.getX(i), vertices.getY(i), 0));
        }
        ...
        circle.position.set(worldX, worldY, worldZ);
    ...
    static createKADText(worldX, worldY, worldZ, text, fontSize, color, backgroundColor = null) {
        ...
        // Step 9) Scale sprite to match text size
        // Estimate text width for proper scaling
        const textWidth = context.measureText(text).width;
        const scale = (textWidth / canvas.width) * fontSize * 2;
