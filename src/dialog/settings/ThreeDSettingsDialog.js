...
export function show3DSettingsDialog() {
	...
	const closestDamping = dampingOptions.reduce((prev, curr) => (Math.abs(curr - currentDamping) < Math.abs(prev - currentDamping) ? curr : prev));
	...
	const fields = [
		{
			...
			options: [{ value: "0", text: "No Spin (0)" }, { value: "0.3", text: "Low (0.3)" }, { value: "0.5", text: "Medium (0.5)" }, { value: "0.7", text: "High (0.7)" }, { value: "1", text: "Max Spin (1)" }]
		},
		{
			type: "select",
			name: "cursorZoom",
		...
		{
			type: "select",
			name: "scrollWheelInverted",
			label: "Scroll wheel forward will:",
			value: currentSettings.scrollWheelInverted === true ? "push" : "pull",
			options: [{ value: "pull", text: "Pull (zoom in)" }, { value: "push", text: "Push (zoom out)" }]
		},
		{
			type: "select",
		...
		},
		{
			...
			options: [{ value: "none", text: "None" }, { value: "pitch", text: "Pitch" }, { value: "roll", text: "Roll" }, { value: "yaw", text: "Yaw" }]
		},
		{
			type: "select",
			name: "gizmoDisplay",
			...
			options: [{ value: "always", text: "Always" }, { value: "only_when_orbit_or_rotate", text: "Only When Orbit or Rotate" }, { value: "never", text: "Never" }]
		},
		{
			type: "select",
			...
			options: [{ value: "off", text: "Off" }, { value: "holes", text: "On (Holes)" }, { value: "kad", text: "On (KAD)" }, { value: "all", text: "On (All)" }]
		}
	];
	...
	const dialog = new FloatingDialog({
		...
		onConfirm: () => {
			...
			// Step 5h.1) Parse scrollWheelInverted from dropdown (string "push"/"pull" to boolean)
			formData.scrollWheelInverted = formData.scrollWheelInverted === "push";

			// Step 5h.2) Parse plumbLineDisplay from dropdown
