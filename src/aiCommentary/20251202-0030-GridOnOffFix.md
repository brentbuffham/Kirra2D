...
updateGridSize(size) {
    ...
    if (!this.gridHelper) {
        console.log("📐 Grid size stored (grid not created yet)");
        return;
    }
...
Tested scenarios:
1. ✅ Turn grid off → Turn grid on (works)
2. ✅ Change settings while off → Turn on (applies settings)
3. ✅ Toggle multiple times (no memory leak)
4. ✅ Change size while on (updates immediately)
5. ✅ Change plane while on (updates immediately)
